...
- (id)init {
    if (self = [super init]) {
        ...
        kSecAttrKeySizeInBitsLength = k1024;
    }
    return self;
}
...
- (void)generateKeyPairOperation{
    @autoreleasepool {
        ...
        [self performSelectorOnMainThread:@selector(generateKeyPairCompleted) withObject:nil waitUntilDone:YES];
    }
}
...
//    key = [self getPrivateKeyRef]; // reejo remove
   ...
   // NSAssert(sanityCheck == noErr, @"Error decrypting, OSStatus == %ld.", (long)sanityCheck);
    ...
    NSString *dataString = [[NSString alloc] initWithData:bits encoding:NSUTF8StringEncoding];
